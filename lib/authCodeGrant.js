...
function checkClient (done) {
  ...
  function isWildcard(clientUri, uri) {
    // Return early.
    if (!/^https:\/\/\*\./.test(clientUri)) {
      return false;
    }

    var re = new RegExp(clientUri.replace(/^(https:\/\/)\*/, '$1[\\w.]+'));
    return re.test(uri);
  }
  ...
  this.model.getClient(this.clientId, null, function (err, client) {
    ...
    if (!client) {
      ...
      if (client.redirectUri.indexOf(self.redirectUri) === -1
        ...
        return isWildcard(validUri, self.redirectUri);
      }).length) {
      ...
      && !isWildcard(client.redirectUri, self.redirectUri)) {
