...
function checkClient (done) {
  ...
  function isChromeAppWildcard(clientUri, uri) {
    // https://ccljkeoplkplgeanjainhelfopgdgkbe.chromiumapp.org/oauth2
    return /^https:\/\/\*\.chromiumapp\.org\/oauth2\/?$/.test(clientUri) &&
          /^https:\/\/[a-z]{32}\.chromiumapp\.org\/oauth2\/?$/.test(uri);
  }
  ...
  this.model.getClient(this.clientId, null, function (err, client) {
    ...
    if (!client) {
      ...
      if (client.redirectUri.indexOf(self.redirectUri) === -1
        && !client.redirectUri.filter(function(validUri) {
        return isChromeAppWildcard(validUri, self.redirectUri);
      }).length) {
      ...
      && !isChromeAppWildcard(client.redirectUri, self.redirectUri)) {
